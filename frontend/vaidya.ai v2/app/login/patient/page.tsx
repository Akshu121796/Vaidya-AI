...
import { Heart, Lock, Mail, ArrowLeft, ArrowRight, User, AlertCircle, Eye, EyeOff } from "lucide-react";
...
export default function PatientLoginPage() {
  ...
  // Empty by default — user must type their own credentials
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setError(result.error || "Invalid credentials. Please check your credentials.");
  };

  const handleDemoLogin = async () => {
    setLoading(true);
    setError("");
    const result = await loginUserAsync("patient@vaidya.ai", "patient123");
    setLoading(false);
    if (result.ok) {
      const user = useHealthStore.getState().user;
      if (user?.role !== "patient") {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 relative overflow-hidden bg-[#04060b]">
      {/* Glow Orbs */}
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[350px] h-[350px] bg-emerald-500/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="w-full max-w-md flex flex-col gap-6 z-10">
        ...
        <Link 
          href="/login" 
          className="self-start text-xs font-semibold text-muted-foreground hover:text-white flex items-center gap-1.5 transition-colors"
        >
        ...
        <div className="text-center flex flex-col items-center gap-2">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-emerald-400 to-teal-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 mb-2">
            <User className="h-5 w-5 text-white" />
          </div>
          <h2 className="text-2xl font-extrabold text-white">Patient Login Portal</h2>
          <p className="text-xs text-muted-foreground">Sign in to your Ayushman Bharat Health Account.</p>
        </div>
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <Input
                type="text"
                placeholder="+91 98765 43210 or patient@vaidya.ai"
                value={phoneOrEmail}
                onChange={(e) => { setPhoneOrEmail(e.target.value); setError(""); }}
          ...
          <Button
            ...
            variant="default"
            disabled={loading}
            className="w-full h-10 rounded-xl font-bold mt-2 flex items-center justify-center gap-1.5"
          >
          ...
          <Button 
            ...
            disabled={loading}
            variant="glow" 
            className="w-full h-10 rounded-xl font-bold flex items-center justify-center gap-1.5 shadow-emerald-500/10"
          >
            Launch Demo Console
          </Button>
          ...
          <p className="text-center text-[9px] text-muted-foreground/50 font-medium">
            Demo: patient@vaidya.ai / patient123
          </p>
        </motion.form>
