...
import { Lock, Mail, ArrowLeft, ArrowRight, Sparkles, AlertCircle, Eye, EyeOff } from "lucide-react";
...
export default function AshaLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.ok) {
      ...
      if (user?.role !== "asha") {
        setError("Unauthorized: This portal is strictly for ASHA Workers.");
        logout();
        return;
      }
  ...
  const handleDemoLogin = async () => {
    setLoading(true);
    setError("");
    const result = await loginUserAsync("asha@vaidya.ai", "asha123");
    setLoading(false);
    if (result.ok) {
      const user = useHealthStore.getState().user;
      if (user?.role !== "asha") {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 relative overflow-hidden bg-[#04060b]">
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[350px] h-[350px] bg-purple-500/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="w-full max-w-md flex flex-col gap-6 z-10">
        ...
        <div className="text-center flex flex-col items-center gap-2">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-purple-400 to-indigo-500 flex items-center justify-center shadow-lg shadow-purple-500/20 mb-2">
            <Sparkles className="h-5 w-5 text-white animate-pulse" />
          </div>
          <h2 className="text-2xl font-extrabold text-white">ASHA worker (ग्रामीण सखी) Gate</h2>
          <p className="text-xs text-muted-foreground">Sign in to sync regional village patient logs.</p>
        </div>
        <motion.form onSubmit={handleLogin} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <Input type="text" placeholder="+91 98765 43210 or asha@vaidya.ai"
          ...
          <Button type="submit" variant="default" disabled={loading} className="w-full h-10 rounded-xl font-bold mt-2 flex items-center justify-center gap-1.5">
            {loading ? "Verifying..." : "Secure Sync Auth"}{!loading && <ArrowRight className="h-4 w-4" />}
          </Button>
          ...
          <Button 
            ...
            disabled={loading}
            variant="glow" 
            className="w-full h-10 rounded-xl font-bold flex items-center justify-center gap-1.5 shadow-purple-500/10"
          >
            Launch Tablet Demo
          </Button>

          <p className="text-center text-[9px] text-muted-foreground/50 font-medium">Demo: asha@vaidya.ai / asha123</p>
        </motion.form>
      </div>
    </div>
  );
