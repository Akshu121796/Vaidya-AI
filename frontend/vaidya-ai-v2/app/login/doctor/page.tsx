...
import { Lock, Mail, ArrowLeft, ArrowRight, Stethoscope, AlertCircle, Eye, EyeOff } from "lucide-react";
...
export default function DoctorLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.ok) {
      ...
      if (user?.role !== "doctor") {
        setError("Unauthorized: This portal is strictly for doctors.");
        logout();
        return;
      }
  ...
  const handleDemoLogin = async () => {
    setLoading(true);
    setError("");
    const result = await loginUserAsync("doctor@vaidya.ai", "doctor123");
    setLoading(false);
    if (result.ok) {
      const user = useHealthStore.getState().user;
      if (user?.role !== "doctor") {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 relative overflow-hidden bg-[#04060b]">
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[350px] h-[350px] bg-cyan-500/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="w-full max-w-md flex flex-col gap-6 z-10">
        ...
        <div className="text-center flex flex-col items-center gap-2">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-cyan-400 to-blue-500 flex items-center justify-center shadow-lg shadow-cyan-500/20 mb-2">
            <Stethoscope className="h-5 w-5 text-white" />
          </div>
          <h2 className="text-2xl font-extrabold text-white">Doctor WorkSpace Gateway</h2>
          <p className="text-xs text-muted-foreground">Sign in to your clinical practitioner panel.</p>
        </div>
        <motion.form onSubmit={handleLogin} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <Input type="text" placeholder="+91 98765 43210 or doctor@vaidya.ai"
                value={phoneOrEmail} onChange={(e) => { setPhoneOrEmail(e.target.value); setError(""); }}
                className="pl-10 h-10 bg-black/40 border-white/5 focus:border-primary/50 text-xs rounded-xl" autoComplete="username" />
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Secure Password</label>
            <div className="relative">
              <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
              <Input type={showPassword ? "text" : "password"} placeholder="••••••••"
                value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }}
                className="pl-10 pr-10 h-10 bg-black/40 border-white/5 focus:border-primary/50 text-xs rounded-xl" autoComplete="current-password" />
              <button
                type="button"
          ...
          <Button type="submit" variant="default" disabled={loading} className="w-full h-10 rounded-xl font-bold mt-2 flex items-center justify-center gap-1.5">
            {loading ? "Verifying..." : "Secure Clinic Auth"}{!loading && <ArrowRight className="h-4 w-4" />}
          </Button>
          ...
          <Button 
            ...
            disabled={loading}
            variant="glow" 
            className="w-full h-10 rounded-xl font-bold flex items-center justify-center gap-1.5 shadow-cyan-500/10"
          >
            Launch Clinic Demo
          </Button>

          <p className="text-center text-[9px] text-muted-foreground/50 font-medium">Demo: doctor@vaidya.ai / doctor123</p>
        </motion.form>
      </div>
    </div>
  );
