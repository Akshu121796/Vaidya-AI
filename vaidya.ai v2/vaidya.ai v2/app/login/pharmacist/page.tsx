...
import { Lock, Mail, ArrowLeft, ArrowRight, Pill, AlertCircle, Eye, EyeOff } from "lucide-react";
...
export default function PharmacistLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.ok) {
      ...
      if (user?.role !== "pharmacist") {
        setError("Unauthorized: This portal is strictly for pharmacists.");
        logout();
        return;
      }
  ...
  const handleDemoLogin = async () => {
    setLoading(true);
    setError("");
    const result = await loginUserAsync("pharmacy@vaidya.ai", "pharmacy123");
    setLoading(false);
    if (result.ok) {
      const user = useHealthStore.getState().user;
      if (user?.role !== "pharmacist") {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 relative overflow-hidden bg-[#04060b]">
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-[350px] h-[350px] bg-amber-500/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="w-full max-w-md flex flex-col gap-6 z-10">
        ...
        <div className="text-center flex flex-col items-center gap-2">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-amber-400 to-orange-500 flex items-center justify-center shadow-lg shadow-amber-500/20 mb-2">
            <Pill className="h-5 w-5 text-white" />
          </div>
          <h2 className="text-2xl font-extrabold text-white">Pharmacy Gateway</h2>
          <p className="text-xs text-muted-foreground">Sign in to manage regional inventories and prescription orders.</p>
        </div>
        <motion.form onSubmit={handleLogin} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <Input type="text" placeholder="+91 98765 43210 or pharmacy@vaidya.ai"
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Store Password</label>
            <div className="relative">
          ...
          <Button type="submit" variant="default" disabled={loading} className="w-full h-10 rounded-xl font-bold mt-2 flex items-center justify-center gap-1.5">
            {loading ? "Verifying..." : "Secure Pharmacy Auth"}{!loading && <ArrowRight className="h-4 w-4" />}
          </Button>
          ...
          <Button 
            ...
            disabled={loading}
            variant="glow" 
            className="w-full h-10 rounded-xl font-bold flex items-center justify-center gap-1.5 shadow-amber-500/10"
          >
            Launch Pharmacy Demo
          </Button>

          <p className="text-center text-[9px] text-muted-foreground/50 font-medium">Demo: pharmacy@vaidya.ai / pharmacy123</p>
        </motion.form>
      </div>
    </div>
  );
